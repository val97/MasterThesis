...
flags.DEFINE_float('small_recommendation_reward', 0.0,
...
# Runner configs.
flags.DEFINE_integer('nsteps', 200, 'Maximum length of a trajectory.') # 1600
...
def run_experiment_Ecoagent(env_config, user_value_model_config, creator_value_model_config,
    ...
    pd.DataFrame(topic_distribution).to_csv("newTest20_10/0.5/topic_distribution_EcoAgent"+post_processing+".csv")
    pd.DataFrame(topic_sum).to_csv("newTest20_10/0.5/topic_sum_EcoAgent"+post_processing+".csv")

    with open('viable_creators_'+post_processing+'.dictionary', 'wb') as viable_creators_file:
        pickle.dump(viable_creators, viable_creators_file)
    #pd.DataFrame(viable_creators).to_csv("newTest20_10/0.5/viable_creators_EcoAgent.csv")
...
def plot_topic_distribution(agent, topic_distribution, experiment, num_interaction, fig):
    ...
    colors = ['red','green','blue','purple', 'cyan', 'yellow', 'black', 'magenta', 'orange', 'pink', 'brown', 'silver', 'violet', 'olive', 'fuchsia']
    ...
    ax = fig.add_subplot(3,4, experiment)
    x = np.arange(1,99,15)
    print("prova", np.array(topic_distribution)[1:99:25,1])
    for topic in range(15):
      #print(topic)
      ax.plot(x, np.array(topic_distribution)[1:99:15,topic], label = str(topic), color= colors[topic])
      ax.set_title(agent + "number of interaction: " + str(num_interaction))
...
def get_document_count_cp(agent, viable_creators, experiment, num_interaction, fig):
    ...
    for cr in viable_creators['creators']:
      ...
      for doc in cr_doc:
          ...
          missing_elem = [ele for ele in range(15) if ele not in topic_per_cp]
    ...
    gone_cp = [ele for ele in range(15) if ele not in present_cp]
    ...
    ind = {'Topic': ['topic1', 'topic2', 'topic3', 'topic4', 'topic5', 'topic6', 'topic7', 'topic8', 'topic9', 'topic10','topic11', 'topic12', 'topic13', 'topic14', 'topic15']}
    ...
    for i in range(10):
      char[i] = np.array(document_count.loc[document_count["creator_id"] == i].sort_values('topic')['topic_count']).reshape(15, 1)
...
def rescaled_topic_distribution_plot(topic_distribution_EcoAgent, topic_distribution_RandomAgent, experiment, num_interaction, fig):
    ...
    for topic in range(15):
      ax.plot(x,np.array(topic_distribution_EcoAgent)[1:99:15,topic], label = str(topic), color= colors[topic])
    ...
    for topic in range(15):
      ax.plot(x, np.array(topic_distribution_RandomAgent)[1:99:15,topic], label = str(topic), color= colors[topic])
...
def run_experiment_RandomAgent(env_config, user_value_model_config, creator_value_model_config,
    ...
    selected_probs.extend(probs['selected_probs'])
    policy_probs.extend(probs['policy_probs'])
    pd.DataFrame(topic_distribution).to_csv("newTest20_10/0.5/topic_distribution_RandomAgent1.csv")
    pd.DataFrame(topic_sum).to_csv("newTest20_10/0.5/topic_sum_RandomAgent1.csv")
    with open('viable_creators_RandomAgent.dictionary', 'wb') as viable_creators_file:
        pickle.dump(viable_creators, viable_creators_file)
...
def analyze_independent_experiment(env_config, user_value_model_config, creator_value_model_config, actor_model_config, exp_config, interaction_vector,  FLAGS, user_ckpt_save_dir, creator_ckpt_save_dir):
    ...
    topicOverCP = PdfPages('newTest20_10/0.5/TopicOverCP.pdf')
    topic_pref = PdfPages('newTest20_10/0.5/TopicPreference.pdf')
    topicOverPostprocessing = PdfPages('newTest20_10/0.5/TopicOverCPpostProcessing.pdf')
    topic_pref_gready = PdfPages('newTest20_10/0.5/TopicPreferencePostProcessing_gready.pdf')
    topic_pref_rebalance =PdfPages('newTest20_10/0.5/TopicPreferencePostProcessing_rebalance.pdf')
    ...
    for n in interaction_vector:                                            #running independent experiment and plotting topic characteristic for each experiment
        ...
        fig2.savefig("newTest20_10/0.5/topic_distribution_over_time.pdf")
        ...
        fig6.savefig('newTest20_10/0.5/viable_creators_EcoAgent.png', dpi=fig.dpi)
        plot_viable_creators("EcoAgent ", viable_creators_RandomAgent, experiment, n, fig6)  #just fig6 is needed as a parameter
        fig7.savefig('newTest20_10/0.5/viable_creators_RandomAgent.png', dpi=fig.dpi)
        ...
        fig2_1.savefig("newTest20_10/0.5/topic_distribution_over_time_balanced_gready0.5.pdf")
        ...
        fig7.savefig('newTest20_10/0.5/viable_creators_EcoAgent_gready.png', dpi=fig.dpi)
        fig8.savefig('newTest20_10/0.5/viable_creators_EcoAgent_rebalanced.png', dpi=fig.dpi)
    ...
    figy.suptitle("Analysis of Long Term Experimemnt over different timestamps " + str(n) , fontsize=13)
    ...
    print(half)
    plot_topic_characteristic_per_timestamp("EcoAgent greedy", topic_sum_EcoAgent_gready, [half], 1 , n, figy.add_subplot(1,2, 1))     #50, 100
    plot_topic_characteristic_per_timestamp("EcoAgent rebalanced ", topic_sum_EcoAgent_rebalance, [half ], 2 , n, figy.add_subplot(1,2, 2))

    plot_topic_characteristic_cp_timestamp("EcoAgent greedy ", viable_creators_EcoAgent_gready, [half], experiment + 1 , n, fig)
    plot_topic_characteristic_cp_timestamp("EcoAgent rebalanced ", viable_creators_EcoAgent_rebalance,[half] , experiment + 1 , n, figx)

    #save_plot_to_pdf(fig, figx, topicOverCP)
    #topicOverCP.close()

    topicPlot = PdfPages('newTest20_10/0.5/TopicPlot_post_processing.pdf')
...
def evaluate_randomAgent(env_config, user_value_model_config, creator_value_model_config, exp_config, post_processing):
  ...
  for rollout in range(50):
       ...
       cp_sat_timesteps = []
       users_sat_timesteps = []
       for time in range(exp_config['nsteps']):
           cp_sat_timesteps = np.append(cp_sat_timesteps, viable_creators.loc[viable_creators['time'] == time]["creator_satisfaction"].sum())
           users_sat_timesteps = np.append(users_sat_timesteps, viable_users.loc[viable_users['time'] == time]["user_satisfaction"].sum())

       u = u.append(pd.DataFrame(users_sat_timesteps).T, ignore_index=True)
       cp = cp.append(pd.DataFrame(cp_sat_timesteps).T, ignore_index=True)
       cp["num_creators"] = num_creators

  cp.loc["mean"] = cp.mean(axis=0)
  u.loc["mean"] = u.mean(axis=0)
  print("cp", cp)
  print("u", u)
  plot_evaluation(u, cp, "newTest20_10/15topics/0.5/evaluation/random/RandomEvaluation"+ str(exp_config["nsteps"])+"_slate"+str(env_config["slate_size"]) + "sc0", exp_config["nsteps"])
  u.to_csv("newTest20_10/15topics/0.5/evaluation/random/RandomUser"+ str(exp_config["nsteps"])+"_slate"+str(env_config["slate_size"])+"sc_0.csv")
  cp.to_csv("newTest20_10/15topics/0.5/evaluation/random/RandomCreator"+str(exp_config["nsteps"])+"_slate"+str(env_config["slate_size"])+"sc_0.csv")
...
def evaluate_EcoAgent(env_config, user_value_model_config, creator_value_model_config, actor_model_config, exp_config, post_processing):
  # Random agent normalization.
  random_user_accumulatFckpted_reward = FLAGS.random_user_accumulated_reward
  random_creator_accumulated_reward = FLAGS.random_creator_accumulated_reward
  ...
  print("cp", cp)
  print("u", u)
  plot_evaluation(u, cp, "newTest20_10/15topics/0.5/evaluation/evaluation"+ str(exp_config["nsteps"])+"_slate"+str(env_config["slate_size"]), exp_config["nsteps"])
  u.to_csv("newTest20_10/15topics/0.5/evaluation/user"+ str(exp_config["nsteps"])+"_slate"+str(env_config["slate_size"])+".csv")
  cp.to_csv("newTest20_10/15topics/0.5/evaluation/creator"+str(exp_config["nsteps"])+"_slate"+str(env_config["slate_size"])+".csv")
...
def get_evaluation_data():
    users_reward = pd.read_csv('newTest20_10/15topics/0.5/evaluation/user100_1.0.csv', index_col=0)
    creator_reward = pd.read_csv('newTest20_10/15topics/0.5/evaluation/creator100_1.0.csv', index_col=0)
    print(users_reward, creator_reward)
    plot_evaluation(users_reward, creator_reward, "newTest20_10/15topics/0/evaluation/Ecoagent_evaluation100_1.0",  100)
...
def plot_evaluation(users_reward, creator_reward, evaluation, steps):
  ...
  print(round(users_reward[19].iloc[-1] ) )
  txt_user = "absolute value user: " + str(round(users_reward[steps-1].iloc[-1] - users_reward[0].iloc[-1],3))
  txt_creator = "absolute value creator: " + str(round(creator_reward[steps-1].iloc[-1] - creator_reward[0].iloc[-1],3))
...
def main(unused_argv):
  ...
  ckpt_save_dir = os.path.join(FLAGS.logdir, 'newTest20_10/15topics/0.5/ckpt_0.5_15topics')
  #ckpt_save_dir = os.path.join(FLAGS.logdir, 'ckpt0.5/')
  ...
  analyze_independent_experiment(env_config, user_value_model_config, creator_value_model_config, actor_model_config, exp_config, [100], FLAGS, user_ckpt_save_dir, creator_ckpt_save_dir)
  #run_experiment_RandomAgent(env_config, user_value_model_config, creator_value_model_config, exp_config, 100, "original")
